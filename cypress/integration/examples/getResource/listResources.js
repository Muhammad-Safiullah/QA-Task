...
When ('user hits a {string} request method for route {string} and page# {int} should have total {int} records per page and expected response Status Code should be {int}', (httpMethod,route,page,recordsPerPage,expectedStatusCode) => {
    cy.readFile(filePath).then((obj) => {
        ...
        url: completeURL, 
            ...
            expect(obj).to.have.keys('id','name','year','color','pantone_value')
                    ...
                    default:
                        cy.request({
                            ...
                            url: completeURL, 
                                ...
                                expect(response.body).have.property('data')
                                expect(response.body.data).to.be.a('array')
                                expect(response.body.data[index]).to.have.any.keys('id','name','year','color','pantone_value')                            
                                expect(response.body).have.property('support')
                                expect(response.body.support).to.deep.equal({
                                    "url": "https://reqres.in/#support-heading",
                                    "text": "To keep ReqRes free, contributions towards server costs are appreciated!"
...
And ('verify that a {string} API route {string} for resourceId {int} should respond returns the data record of only that resource and the expected response code should be {int}', (httpMethod,route,userId,expectedStatusCode) => {
    cy.readFile(filePath).then((obj) => {
        var mainURL = obj.url
        var completeURL = mainURL.concat(route)
        var mainURL = obj.url
...
Then ('verify that if the record doesnot exist for resourceId {int} the empty body should be the response with {int} as response code', (userId,expectedStatusCode) => {
    cy.readFile(filePath).then((obj) => {
        var httpMethod = 'GET'
        var route = '/api/unknown'
        var mainURL = obj.url
        var completeURL = mainURL.concat(route)
        var mainURL = obj.url
        completeURL = mainURL.concat(route).concat('/').concat(userId)
